...
import { validateSubCategoryInput, validateSubCategoryUpdateInput } from '../helpers/validators/SubCategoryValidator';
...
export const updateSubCategoryById = async (req: Request, res: Response) => {
  const { error } = validateSubCategoryUpdateInput(req.body);
  if (error) return res.status(400).json({ error: error.details[0].message });
